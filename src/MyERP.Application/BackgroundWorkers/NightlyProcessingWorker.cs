    ...
    protected override async Task DoWorkAsync(PeriodicBackgroundWorkerContext workerContext)
    {
        ...
        foreach (var company in companies)
        {
            try
            {
                ...
                // Enqueue employee loan repayment deductions and overdue penalties
                // Per ERPNext: loan_repayment.process_payroll_deduction
                await jobManager.EnqueueAsync(new HumanResources.BackgroundJobs.EmployeeLoanDeductionJobArgs
                {
                    CompanyId = company.Id,
                    TenantId = company.TenantId,
                    AsOfDate = DateTime.UtcNow.Date,
                ...
                // Enqueue blanket order validity and fulfillment monitoring
                // Per ERPNext: blanket_order.update_blanket_order_status
                await jobManager.EnqueueAsync(new Sales.BackgroundJobs.BlanketOrderExpiryJobArgs
                {
                    CompanyId = company.Id,
                    TenantId = company.TenantId,
                    AsOfDate = DateTime.UtcNow.Date,
                ...
                // Enqueue unbilled timesheet tracking and reminders
                // Per ERPNext: timesheet.update_billing_status
                await jobManager.EnqueueAsync(new Projects.BackgroundJobs.TimesheetBillingStatusJobArgs
                {
                    CompanyId = company.Id,
                    TenantId = company.TenantId,
                    AsOfDate = DateTime.UtcNow.Date,
                ...
                // Enqueue asset maintenance planned log generation and task due date scheduling
                // Per ERPNext: asset_maintenance.update_maintenance_status
                await jobManager.EnqueueAsync(new Maintenance.BackgroundJobs.AssetMaintenanceAutoLogJobArgs
                {
                    CompanyId = company.Id,
                    TenantId = company.TenantId,
                    AsOfDate = DateTime.UtcNow.Date,
                ...
                // Enqueue supplier performance scorecard evaluation and PO enforcement
                // Per ERPNext: supplier_scorecard.evaluate_scorecards
        ...
        logger.LogInformation("NightlyProcessingWorker: Enqueued {Count} companies for nightly processing (61 jobs).", companies.Count);
    }
}
