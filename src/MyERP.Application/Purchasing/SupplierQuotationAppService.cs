...
public class SupplierQuotationAppService : ApplicationService, ISupplierQuotationAppService
{
    ...
    public async Task<SupplierQuotationDto> CreateAsync(CreateSupplierQuotationDto input)
    {
        ...
        await LazyServiceProvider.LazyGetRequiredService<PartyValidationService>()
            .ValidatePartyForTransactionAsync("Supplier", isDisabled: !supplier.IsActive, isFrozen: supplier.IsFrozen, supplier.Name, input.CompanyId);
        ...
        if (input.RequestForQuotationId.HasValue)
        {
            ...
            if (rfq != null && rfq.CompanyId != input.CompanyId)
            {
                throw new Volo.Abp.BusinessException(MyERPDomainErrorCodes.CompanyMismatch)
                    .WithData("rfqCompany", rfq.CompanyId)
                    .WithData("supplierQuotationCompany", input.CompanyId);
            }
