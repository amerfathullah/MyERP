...
type AssetMovementEntity = AssetMovementDto & { id: string };

export const AssetMovementStore = signalStore(
  { providedIn: 'root' },
  withState({ totalCount: 0, isLoading: false }),
  withEntities<AssetMovementEntity>(),
  withMethods((store, service = inject(AssetMovementService), toaster = inject(ToasterService)) => ({
    load: rxMethod<any>(
      pipe(
        tap(() => patchState(store, { isLoading: true })),
        switchMap((query) => service.getList(query)),
        tap((result) => {
          patchState(store, setAllEntities((result.items ?? []) as AssetMovementEntity[], { selectId: (e) => e.id! }));
    ...
    create: rxMethod<any>(
      pipe(
        ...
        tap((created) => { patchState(store, addEntity(created as AssetMovementEntity, { selectId: (e) => e.id! })); toaster.success('::SuccessfullyCreated'); }),
    ...
    submit: rxMethod<string>(
      pipe(
        ...
        tap((updated) => { patchState(store, updateEntity({ id: (updated as AssetMovementEntity).id!, changes: updated as AssetMovementEntity })); toaster.success('::Submitted'); }),
    ...
    cancel: rxMethod<string>(
      pipe(
        ...
        tap((updated) => { patchState(store, updateEntity({ id: (updated as AssetMovementEntity).id!, changes: updated as AssetMovementEntity })); toaster.success('::Cancelled'); }),
