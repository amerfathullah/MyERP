import type { BulkTransactionLogDto, CreateBulkTransactionLogDto, GetBulkTransactionLogListDto, RecordBulkTransactionResultDto } from './dtos/models';
...
export class BulkTransactionLogService {
  ...
  recordDetailResult = (id: string, detailId: string, input: RecordBulkTransactionResultDto, config?: Partial<Rest.Config>) =>
    this.restService.request<any, BulkTransactionLogDto>({
      method: 'POST',
      url: `/api/app/bulk-transaction-log/${id}/detail/${detailId}/record-result`,
      body: input,
    },
    { apiName: this.apiName, ...config });
  ...
  retryDetail = (id: string, detailId: string, config?: Partial<Rest.Config>) =>
    this.restService.request<any, BulkTransactionLogDto>({
      method: 'POST',
      url: `/api/app/bulk-transaction-log/${id}/detail/${detailId}/retry`,
    },
    { apiName: this.apiName, ...config });
}
