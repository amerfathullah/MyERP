import type { ApprovalRequestDto, ApprovalRuleDto, CreateApprovalRuleDto, ReviewApprovalDto, UpdateApprovalRuleDto } from './dtos/models';
...
export class ApprovalWorkflowService {
  ...
  deleteRule = (id: string, config?: Partial<Rest.Config>) =>
    this.restService.request<any, void>({
      method: 'DELETE',
      url: `/api/app/approval-workflow/${id}/rule`,
    },
    { apiName: this.apiName,...config });
  

  getDocumentApprovals = (documentType: string, documentId: string, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<ApprovalRequestDto>>({
      method: 'GET',
      url: `/api/app/approval-workflow/document-approvals/${documentId}`,
      params: { documentType },
    },
    { apiName: this.apiName,...config });
