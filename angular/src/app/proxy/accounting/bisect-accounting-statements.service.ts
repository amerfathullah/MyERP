import type {
  BisectAccountingStatementsDto,
  BisectAccountingStatementsGetListInput,
  CreateBisectAccountingStatementsDto
} from './models';
...
export class BisectAccountingStatementsService {
  ...
  createAndBuildTree = (input: CreateBisectAccountingStatementsDto, config?: Partial<Rest.Config>) =>
    this.restService.request<any, BisectAccountingStatementsDto>({
      method: 'POST',
      url: '/api/app/bisect-accounting-statements/create-and-build-tree',
      body: input,
    },
    { apiName: this.apiName, ...config });
