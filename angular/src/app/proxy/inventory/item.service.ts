...
export class ItemService {
  ...
  getList = (input: GetItemListDto, config?: Partial<Rest.Config>) =>
    this.restService.request<any, PagedResultDto<ItemDto>>({
      method: 'GET',
      url: '/api/app/item',
      params: { filter: input.filter, companyId: input.companyId, itemType: input.itemType, customerId: input.customerId, supplierId: input.supplierId, hasVariants: input.hasVariants, hasBatchNo: input.hasBatchNo, hasSerialNo: input.hasSerialNo, maintainStock: input.maintainStock, sorting: input.sorting, skipCount: input.skipCount, maxResultCount: input.maxResultCount },
    },
    { apiName: this.apiName,...config });
