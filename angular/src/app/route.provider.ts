import { RoutesService, eLayoutType } from '@abp/ng.core';
import { inject, provideAppInitializer } from '@angular/core';
export const APP_ROUTE_PROVIDER = [
  provideAppInitializer(() => {
    configureRoutes();
  })];
function configureRoutes() {
  const routes = inject(RoutesService);
  routes.add([
      {
        path: '/',
        name: '::Menu:Home',
        iconClass: 'fas fa-home',
        order: 1,
        layout: eLayoutType.application,
      },
      {
        path: '/companies',
        name: '::Menu:Companies',
        iconClass: 'fas fa-building',
        order: 2,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Companies',
      },
      {
        path: '/customers',
        name: '::Menu:Customers',
        iconClass: 'fas fa-users',
        order: 3,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Customers',
      },
      {
        path: '/sales',
        name: '::Menu:Sales',
        iconClass: 'fas fa-shopping-cart',
        order: 10,
        layout: eLayoutType.application,
      },
      {
        path: '/sales/quotations',
        name: '::Menu:Quotations',
        parentName: '::Menu:Sales',
        iconClass: 'fas fa-file-lines',
        order: 1,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Quotations',
      },
      {
        path: '/sales/orders',
        name: '::Menu:SalesOrders',
        parentName: '::Menu:Sales',
        iconClass: 'fas fa-clipboard-list',
        order: 2,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesOrders',
      },
      {
        path: '/sales/tracking-board',
        name: '::Menu:SOTrackingBoard',
        parentName: '::Menu:Sales',
        iconClass: 'fas fa-columns',
        order: 3,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesOrders',
      },
      {
        path: '/sales/invoices',
        name: '::Menu:SalesInvoices',
        parentName: '::Menu:Sales',
        iconClass: 'fas fa-file-invoice-dollar',
        order: 3,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesInvoices',
      },
      {
        path: '/sales/delivery-notes',
        name: '::Menu:DeliveryNotes',
        parentName: '::Menu:Sales',
        iconClass: 'fas fa-truck-fast',
        order: 4,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.DeliveryNotes',
      },
      {
        path: '/sales/pos',
        name: '::Menu:POS',
        parentName: '::Menu:Sales',
        iconClass: 'fas fa-cash-register',
        order: 10,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesInvoices.Create',
      },
      {
        path: '/sales/pos-closing',
        name: '::Menu:POSClosing',
        parentName: '::Menu:Sales',
        iconClass: 'fas fa-register',
        order: 11,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesInvoices',
      },
      {
        path: '/sales/pos-opening',
        name: '::Menu:POSOpening',
        parentName: '::Menu:Sales',
        iconClass: 'fas fa-door-open',
        order: 12,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesInvoices',
      },
      {
        path: '/sales/product-bundles',
        name: '::Menu:ProductBundles',
        parentName: '::Menu:Sales',
        iconClass: 'fas fa-cubes',
        order: 13,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Items',
      },
      {
        path: '/sales/loyalty-programs',
        name: '::Menu:LoyaltyPrograms',
        parentName: '::Menu:Sales',
        iconClass: 'fas fa-gift',
        order: 12,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.LoyaltyPrograms',
      },
      {
        path: '/sales/coupon-codes',
        name: '::Menu:CouponCodes',
        parentName: '::Menu:Sales',
        iconClass: 'fas fa-ticket',
        order: 13,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesInvoices',
      },
      {
        path: '/sales/party-specific-items',
        name: '::Menu:PartySpecificItems',
        parentName: '::Menu:Sales',
        iconClass: 'fas fa-filter',
        order: 14,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PartySpecificItems',
      },
      {
        path: '/sales/packing-slips',
        name: '::Menu:PackingSlips',
        parentName: '::Menu:Sales',
        iconClass: 'fas fa-box',
        order: 14,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.DeliveryNotes',
      },
      {
        path: '/sales/shipping-rules',
        name: '::Menu:ShippingRules',
        parentName: '::Menu:Sales',
        iconClass: 'fas fa-truck-fast',
        order: 15,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.ShippingRules',
      },
      {
        path: '/sales/shipments',
        name: '::Menu:Shipments',
        parentName: '::Menu:Sales',
        iconClass: 'fas fa-truck',
        order: 16,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesOrders',
      },
      {
        path: '/sales/sales-persons',
        name: '::Menu:SalesPersons',
        parentName: '::Menu:Sales',
        iconClass: 'fas fa-user-tie',
        order: 14,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesPersons',
      },
      {
        path: '/sales/sales-partners',
        name: '::Menu:SalesPartners',
        parentName: '::Menu:Sales',
        iconClass: 'fas fa-handshake',
        order: 15,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesPartners',
      },
      {
        path: '/sales/sales-partner-types',
        name: '::Menu:SalesPartnerTypes',
        parentName: '::Menu:Sales',
        iconClass: 'fas fa-tags',
        order: 16,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesPartnerTypes',
      },
      {
        path: '/maintenance/warranty-claims',
        name: '::Menu:WarrantyClaims',
        parentName: '::Menu:Support',
        iconClass: 'fas fa-shield-halved',
        order: 3,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.WarrantyClaims',
      },
      {
        path: '/suppliers',
        name: '::Menu:Suppliers',
        iconClass: 'fas fa-truck',
        order: 5,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Suppliers',
      },
      {
        path: '/e-invoice',
        name: '::Menu:EInvoice',
        iconClass: 'fas fa-file-signature',
        order: 20,
        layout: eLayoutType.application,
      },
      {
        path: '/e-invoice/dashboard',
        name: '::Menu:LhdnDashboard',
        parentName: '::Menu:EInvoice',
        iconClass: 'fas fa-chart-pie',
        order: 1,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.EInvoice',
      },
      {
        path: '/e-invoice/logs',
        name: '::Menu:SubmissionLogs',
        parentName: '::Menu:EInvoice',
        iconClass: 'fas fa-list-check',
        order: 2,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.EInvoice',
      },
      {
        path: '/e-invoice/reports',
        name: '::Menu:EInvoiceStatusReport',
        parentName: '::Menu:EInvoice',
        iconClass: 'fas fa-table-list',
        order: 3,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.EInvoice',
      },
      {
        path: '/purchasing',
        name: '::Menu:Purchasing',
        iconClass: 'fas fa-cart-shopping',
        order: 15,
        layout: eLayoutType.application,
      },
      {
        path: '/purchasing/orders',
        name: '::Menu:PurchaseOrders',
        parentName: '::Menu:Purchasing',
        iconClass: 'fas fa-clipboard-list',
        order: 1,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PurchaseOrders',
      },
      {
        path: '/purchasing/incoterms',
        name: '::Menu:Incoterms',
        parentName: '::Menu:Purchasing',
        iconClass: 'fas fa-ship',
        order: 90,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PurchaseOrders',
      },
      {
        path: '/purchasing/invoices',
        name: '::Menu:PurchaseInvoices',
        parentName: '::Menu:Purchasing',
        iconClass: 'fas fa-file-invoice',
        order: 2,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PurchaseInvoices',
      },
      {
        path: '/purchasing/receipts',
        name: '::Menu:PurchaseReceipts',
        parentName: '::Menu:Purchasing',
        iconClass: 'fas fa-box-open',
        order: 3,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PurchaseReceipts',
      },
      {
        path: '/purchasing/dashboard',
        name: '::Menu:ProcurementDashboard',
        parentName: '::Menu:Purchasing',
        iconClass: 'fas fa-gauge-high',
        order: 1,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PurchaseOrders',
      },
      {
        path: '/purchasing/tracking-board',
        name: '::Menu:POTrackingBoard',
        parentName: '::Menu:Purchasing',
        iconClass: 'fas fa-columns',
        order: 2,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PurchaseOrders',
      },
      {
        path: '/purchasing/material-requests',
        name: '::Menu:MaterialRequests',
        parentName: '::Menu:Purchasing',
        iconClass: 'fas fa-clipboard-list',
        order: 4,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.MaterialRequests',
      },
      {
        path: '/purchasing/subcontracting',
        name: '::Menu:Subcontracting',
        parentName: '::Menu:Purchasing',
        iconClass: 'fas fa-industry',
        order: 5,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PurchaseOrders',
      },
      {
        path: '/purchasing/subcontracting-inward',
        name: '::Menu:SubcontractingInwardOrders',
        parentName: '::Menu:Purchasing',
        iconClass: 'fas fa-truck-ramp-box',
        order: 6,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PurchaseOrders',
      },
      {
        path: '/purchasing/subcontracting-boms',
        name: '::Menu:SubcontractingBoms',
        parentName: '::Menu:Purchasing',
        iconClass: 'fas fa-diagram-project',
        order: 7,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PurchaseOrders',
      },
      {
        path: '/purchasing/scorecards',
        name: '::Menu:SupplierScorecards',
        parentName: '::Menu:Purchasing',
        iconClass: 'fas fa-star-half-stroke',
        order: 7,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SupplierScorecards',
      },
      {
        path: '/purchasing/scorecard-variables',
        name: '::Menu:SupplierScorecardVariables',
        parentName: '::Menu:Purchasing',
        iconClass: 'fas fa-code',
        order: 7.5,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SupplierScorecardVariables',
      },
      {
        path: '/purchasing/reports/upcoming-payments',
        name: '::Menu:UpcomingPaymentsDue',
        parentName: '::Menu:Purchasing',
        iconClass: 'fas fa-calendar-days',
        order: 15,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PurchaseInvoices',
      },
      {
        path: '/purchasing/reports/supplier-payment-summary',
        name: '::Menu:SupplierPaymentSummary',
        parentName: '::Menu:Purchasing',
        iconClass: 'fas fa-chart-pie',
        order: 16,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PurchaseInvoices',
      },
      {
        path: '/purchasing/reports/delivery-performance',
        name: '::Menu:SupplierDeliveryPerformance',
        parentName: '::Menu:Purchasing',
        iconClass: 'fas fa-truck-clock',
        order: 17,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PurchaseOrders',
      },
      {
        path: '/purchasing/reports/analytics',
        name: '::Menu:PurchaseAnalytics',
        parentName: '::Menu:Purchasing',
        iconClass: 'fas fa-chart-bar',
        order: 18,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PurchaseInvoices',
      },
      {
        path: '/purchasing/reports/fulfillment',
        name: '::Menu:PoFulfillmentReport',
        parentName: '::Menu:Purchasing',
        iconClass: 'fas fa-clipboard-check',
        order: 19,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PurchaseOrders',
      },
      {
        path: '/accounting',
        name: '::Menu:Accounting',
        iconClass: 'fas fa-calculator',
        order: 30,
        layout: eLayoutType.application,
      },
      {
        path: '/accounting/accounts',
        name: '::Menu:ChartOfAccounts',
        parentName: '::Menu:Accounting',
        iconClass: 'fas fa-sitemap',
        order: 1,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/journal-entries/new',
        name: '::Menu:JournalEntry',
        parentName: '::Menu:Accounting',
        iconClass: 'fas fa-book',
        order: 2,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.JournalEntries.Create',
      },
      {
        path: '/accounting/payments/new',
        name: '::Menu:PaymentEntry',
        parentName: '::Menu:Accounting',
        iconClass: 'fas fa-money-bill-transfer',
        order: 3,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PaymentEntries.Create',
      },
      {
        path: '/accounting/bank-guarantees',
        name: '::Menu:BankGuarantees',
        parentName: '::Menu:Accounting',
        iconClass: 'fas fa-shield-halved',
        order: 4,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.BankGuarantees',
      },
      {
        path: '/accounting/reports/trial-balance',
        name: '::Menu:TrialBalance',
        parentName: '::Menu:Accounting',
        iconClass: 'fas fa-balance-scale',
        order: 10,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/reports/profit-loss',
        name: '::Menu:ProfitLoss',
        parentName: '::Menu:Accounting',
        iconClass: 'fas fa-chart-line',
        order: 11,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/reports/pl-by-cost-center',
        name: '::Menu:PLByCostCenter',
        parentName: '::Menu:Accounting',
        iconClass: 'fas fa-diagram-project',
        order: 12,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/reports/monthly-profit-loss',
        name: '::Menu:MonthlyProfitLoss',
        parentName: '::Menu:Accounting',
        iconClass: 'fas fa-table-columns',
        order: 13,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/reports/balance-sheet',
        name: '::Menu:BalanceSheet',
        parentName: '::Menu:Accounting',
        iconClass: 'fas fa-scale-balanced',
        order: 12,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/bank-reconciliation',
        name: '::Menu:BankReconciliation',
        parentName: '::Menu:Accounting',
        iconClass: 'fas fa-building-columns',
        order: 20,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PaymentEntries',
      },
      {
        path: '/accounting/bank-reconciliation-statement',
        name: '::Menu:BankReconciliationStatement',
        parentName: '::Menu:Accounting',
        iconClass: 'fas fa-file-lines',
        order: 21,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PaymentEntries',
      },
      {
        path: '/accounting/bank-clearance',
        name: '::Menu:BankClearance',
        parentName: '::Menu:Accounting',
        iconClass: 'fas fa-money-check-dollar',
        order: 22,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PaymentEntries',
      },
      {
        path: '/accounting/payment-reconciliation',
        name: '::Menu:PaymentReconciliation',
        parentName: '::Menu:Accounting',
        iconClass: 'fas fa-handshake',
        order: 21,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PaymentEntries',
      },
      {
        path: '/accounting/budgets',
        name: '::Menu:Budgets',
        parentName: '::Menu:Accounting',
        iconClass: 'fas fa-calculator',
        order: 21,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Budgets',
      },
      {
        path: '/accounting/monthly-distributions',
        name: '::Menu:MonthlyDistributions',
        parentName: '::Menu:Accounting',
        iconClass: 'fas fa-bar-chart',
        order: 21,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Budgets',
      },
      {
        path: '/accounting/shareholders',
        name: '::Menu:Shareholders',
        parentName: '::Menu:Accounting',
        iconClass: 'fas fa-users',
        order: 22,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.ShareManagement',
      },
      {
        path: '/accounting/share-transfers',
        name: '::Menu:ShareTransfers',
        parentName: '::Menu:Accounting',
        iconClass: 'fas fa-right-left',
        order: 23,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.ShareManagement',
      },
      {
        path: '/inventory',
        name: '::Menu:Inventory',
        iconClass: 'fas fa-warehouse',
        order: 35,
        layout: eLayoutType.application,
      },
      {
        path: '/inventory/items',
        name: '::Menu:Items',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-box',
        order: 1,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Items',
      },
      {
        path: '/inventory/item-lead-times',
        name: '::MenuItemLeadTimes',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-stopwatch',
        order: 2,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.ItemLeadTimes',
      },
      {
        path: '/inventory/brands',
        name: '::Menu:Brands',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-certificate',
        order: 3,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Items',
      },
      {
        path: '/inventory/uom-categories',
        name: '::Menu:UomCategories',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-ruler',
        order: 90,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Items',
      },
      {
        path: '/inventory/warehouses',
        name: '::Menu:Warehouses',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-building-columns',
        order: 2,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Warehouses',
      },
      {
        path: '/inventory/stock-entries',
        name: '::Menu:StockEntries',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-right-left',
        order: 3,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.StockEntries',
      },
      {
        path: '/inventory/reports/stock-ledger',
        name: '::Menu:StockLedger',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-scroll',
        order: 10,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Items',
      },
      {
        path: '/inventory/reports/stock-balance',
        name: '::Menu:StockBalance',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-cubes',
        order: 11,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Items',
      },
      {
        path: '/inventory/quality-inspections',
        name: '::Menu:QualityInspections',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-clipboard-check',
        order: 4,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.QualityInspections',
      },
      {
        path: '/inventory/quality-goals',
        name: '::Menu:QualityGoals',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-bullseye',
        order: 5,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.QualityGoals',
      },
      {
        path: '/inventory/quality-reviews',
        name: '::Menu:QualityReviews',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-clipboard-list',
        order: 6,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.QualityReviews',
      },
      {
        path: '/inventory/quality-procedures',
        name: '::Menu:QualityProcedures',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-sitemap',
        order: 7,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.QualityProcedures',
      },
      {
        path: '/inventory/quality-actions',
        name: '::Menu:QualityActions',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-tasks',
        order: 8,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.QualityActions',
      },
      {
        path: '/inventory/non-conformances',
        name: '::Menu:NonConformances',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-triangle-exclamation',
        order: 9,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.NonConformances',
      },
      {
        path: '/inventory/quality-meetings',
        name: '::Menu:QualityMeetings',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-people-group',
        order: 10,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.QualityMeetings',
      },
      {
        path: '/inventory/quality-inspection-templates',
        name: '::Menu:QualityInspectionTemplates',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-clipboard-list',
        order: 11,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.QualityInspections',
      },
      {
        path: '/inventory/quality-inspection-parameter-groups',
        name: '::Menu:QualityInspectionParameterGroups',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-layer-group',
        order: 12,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.QualityInspectionParameterGroups',
      },
      {
        path: '/inventory/quality-feedback',
        name: '::Menu:QualityFeedbacks',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-comment-dots',
        order: 13,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.QualityFeedbacks',
      },
      {
        path: '/inventory/stock-reconciliations',
        name: '::Menu:StockReconciliations',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-scale-balanced',
        order: 5,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.StockReconciliations',
      },
      {
        path: '/inventory/landed-costs',
        name: '::Menu:LandedCostVouchers',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-truck-ramp-box',
        order: 6,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.LandedCostVouchers',
      },
      {
        path: '/inventory/stock-reservations',
        name: '::Menu:StockReservations',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-lock',
        order: 7,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.StockEntries',
      },
      {
        path: '/inventory/pick-lists',
        name: '::Menu:PickLists',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-clipboard-list',
        order: 8,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.StockEntries',
      },
      {
        path: '/inventory/transit-transfers',
        name: '::Menu:TransitTransfers',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-truck-moving',
        order: 9,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.StockEntries',
      },
      {
        path: '/inventory/standard-costs',
        name: '::Menu:ItemStandardCosts',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-tag',
        order: 9,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.StockEntries',
      },
      {
        path: '/inventory/item-prices',
        name: '::Menu:ItemPrices',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-tags',
        order: 10,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Items',
      },
      {
        path: '/inventory/customs-tariff-numbers',
        name: '::Menu:CustomsTariffNumbers',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-tag',
        order: 10,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.CustomsTariffNumbers',
      },
      {
        path: '/inventory/manufacturers',
        name: '::Menu:Manufacturers',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-industry',
        order: 11,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Manufacturers',
      },
      {
        path: '/inventory/item-alternatives',
        name: '::Menu:ItemAlternatives',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-shuffle',
        order: 12,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.ItemAlternatives',
      },
      {
        path: '/inventory/delivery-trips',
        name: '::Menu:DeliveryTrips',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-truck-fast',
        order: 13,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.DeliveryTrips',
      },
      {
        path: '/inventory/shipment-parcel-templates',
        name: '::Menu:ShipmentParcelTemplates',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-box-open',
        order: 14,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.ShipmentParcelTemplates',
      },
      {
        path: '/inventory/repost-valuation',
        name: '::Menu:RepostItemValuation',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-rotate',
        order: 10,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.StockEntries',
      },
      {
        path: '/inventory/stock-closing',
        name: '::Menu:StockClosing',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-box-archive',
        order: 11,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.StockEntries',
      },
      {
        path: '/inventory/stock-reorder',
        name: '::Menu:StockReorder',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-cart-arrow-down',
        order: 15,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.StockEntries',
      },
      {
        path: '/inventory/reports/aging',
        name: '::Menu:InventoryAging',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-hourglass-half',
        order: 12,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.StockEntries',
      },
      {
        path: '/inventory/reports/turnover',
        name: '::Menu:InventoryTurnover',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-rotate',
        order: 13,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.StockEntries',
      },
      {
        path: '/inventory/reports/reorder-suggestions',
        name: '::Menu:ReorderSuggestions',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-chart-line',
        order: 15,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Items',
      },
      {
        path: '/inventory/reports/batch-traceability',
        name: '::Menu:BatchTraceability',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-route',
        order: 15,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Items',
      },
      {
        path: '/inventory/reports/stock-movement',
        name: '::Menu:StockMovementSummary',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-exchange-alt',
        order: 14,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.StockEntries',
      },
      {
        path: '/inventory/reports/stock-gl-comparison',
        name: '::Menu:StockGlComparison',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-not-equal',
        order: 15,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.StockEntries',
      },
      {
        path: '/inventory/reports/reorder-point',
        name: '::Menu:ReorderPoint',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-bell-exclamation',
        order: 15,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.StockEntries',
      },
      {
        path: '/inventory/reports/batch-wise-balance',
        name: '::Menu:BatchWiseBalance',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-boxes-stacked',
        order: 15,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.StockEntries',
      },
      {
        path: '/inventory/item-attributes',
        name: '::Menu:ItemAttributes',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-palette',
        order: 11,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Items',
      },
      {
        path: '/inventory/qi-templates',
        name: '::Menu:QualityInspectionTemplates',
        parentName: '::Menu:Inventory',
        iconClass: 'fas fa-clipboard-check',
        order: 12,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.QualityInspections',
      },
      {
        path: '/workflow',
        name: '::Menu:Workflow',
        iconClass: 'fas fa-list-check',
        order: 40,
        layout: eLayoutType.application,
      },
      {
        path: '/workflow/pending',
        name: '::Menu:PendingApprovals',
        parentName: '::Menu:Workflow',
        iconClass: 'fas fa-clock',
        order: 1,
        layout: eLayoutType.application,
      },
      {
        path: '/workflow/rules',
        name: '::Menu:ApprovalRules',
        parentName: '::Menu:Workflow',
        iconClass: 'fas fa-gears',
        order: 2,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.ApprovalWorkflows',
      },
      {
        path: '/import-export',
        name: '::Menu:ImportExport',
        iconClass: 'fas fa-file-import',
        order: 50,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.ImportExport',
      },
      {
        path: '/automation',
        name: '::Menu:Automation',
        iconClass: 'fas fa-robot',
        order: 45,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.AutomationRules',
      },
      {
        path: '/hr',
        name: '::Menu:HR',
        iconClass: 'fas fa-users',
        order: 50,
        layout: eLayoutType.application,
      },
      {
        path: '/hr/employees',
        name: '::Menu:Employees',
        parentName: '::Menu:HR',
        iconClass: 'fas fa-user-tie',
        order: 1,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Employees',
      },
      {
        path: '/hr/designations',
        name: '::Menu:Designations',
        parentName: '::Menu:HR',
        iconClass: 'fas fa-id-badge',
        order: 2,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Employees',
      },
      {
        path: '/hr/departments',
        name: '::Menu:Departments',
        parentName: '::Menu:HR',
        iconClass: 'fas fa-sitemap',
        order: 3,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Employees',
      },
      {
        path: '/hr/employee-groups',
        name: '::Menu:EmployeeGroups',
        parentName: '::Menu:HR',
        iconClass: 'fas fa-users-rectangle',
        order: 4,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.EmployeeGroups',
      },
      {
        path: '/hr/payroll',
        name: '::Menu:Payroll',
        parentName: '::Menu:HR',
        iconClass: 'fas fa-money-check-alt',
        order: 2,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Payroll',
      },
      {
        path: '/tax/categories',
        name: '::Menu:TaxCategories',
        iconClass: 'fas fa-percent',
        order: 55,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.TaxCategories',
      },
      {
        path: '/tax/summary',
        name: '::Menu:TaxSummary',
        iconClass: 'fas fa-file-invoice-dollar',
        order: 56,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.TaxCategories',
      },
      {
        path: '/tax/sst02-filing',
        name: '::Menu:SST02Filing',
        iconClass: 'fas fa-file-circle-check',
        order: 57,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.TaxCategories',
      },
      {
        path: '/tax/templates',
        name: '::Menu:TaxChargesTemplates',
        iconClass: 'fas fa-receipt',
        order: 58,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.TaxTemplates',
      },
      {
        path: '/tax/item-tax-templates',
        name: '::Menu:ItemTaxTemplates',
        iconClass: 'fas fa-tags',
        order: 59,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.TaxCategories',
      },
      {
        path: '/tax/tax-withholding-groups',
        name: '::Menu:TaxWithholdingGroups',
        iconClass: 'fas fa-users-cog',
        order: 59.5,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.TaxWithholdingGroups',
      },
      {
        path: '/tax/withholding-categories',
        name: '::Menu:TaxWithholdingCategories',
        iconClass: 'fas fa-percentage',
        order: 60,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.TaxCategories',
      },
      {
        path: '/tax/lower-deduction-certificates',
        name: '::Menu:LowerDeductionCertificates',
        iconClass: 'fas fa-certificate',
        order: 61,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.TaxCategories',
      },
      {
        path: '/tax/rules',
        name: '::Menu:TaxRules',
        iconClass: 'fas fa-gavel',
        order: 62,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.TaxCategories',
      },
      {
        path: '/crm',
        name: '::Menu:CRM',
        iconClass: 'fas fa-handshake',
        order: 8,
        layout: eLayoutType.application,
      },
      {
        path: '/crm/leads',
        name: '::Menu:Leads',
        parentName: '::Menu:CRM',
        iconClass: 'fas fa-user-plus',
        order: 1,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Leads',
      },
      {
        path: '/crm/opportunities',
        name: '::Menu:Opportunities',
        parentName: '::Menu:CRM',
        iconClass: 'fas fa-lightbulb',
        order: 2,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Opportunities',
      },
      {
        path: '/crm/sales-pipeline',
        name: '::Menu:SalesPipeline',
        parentName: '::Menu:CRM',
        iconClass: 'fas fa-filter',
        order: 3,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Opportunities',
      },
      {
        path: '/crm/prospects',
        name: '::Menu:Prospects',
        parentName: '::Menu:CRM',
        iconClass: 'fas fa-building',
        order: 3,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Leads',
      },
      {
        path: '/crm/contracts',
        name: '::Menu:Contracts',
        parentName: '::Menu:CRM',
        iconClass: 'fas fa-file-contract',
        order: 4,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Leads',
      },
      {
        path: '/crm/appointments',
        name: '::Menu:Appointments',
        parentName: '::Menu:CRM',
        iconClass: 'fas fa-calendar-check',
        order: 5,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Leads',
      },
      {
        path: '/crm/campaigns',
        name: '::Menu:Campaigns',
        parentName: '::Menu:CRM',
        iconClass: 'fas fa-bullhorn',
        order: 6,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Leads',
      },
      {
        path: '/crm/email-campaigns',
        name: '::Menu:EmailCampaigns',
        parentName: '::Menu:CRM',
        iconClass: 'fas fa-envelope-open-text',
        order: 7,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Leads',
      },
      {
        path: '/crm/contract-templates',
        name: '::Menu:ContractTemplates',
        parentName: '::Menu:CRM',
        iconClass: 'fas fa-file-signature',
        order: 8,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Leads',
      },
      {
        path: '/crm/competitors',
        name: '::Menu:Competitors',
        parentName: '::Menu:CRM',
        iconClass: 'fas fa-chess',
        order: 9,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Leads',
      },
      {
        path: '/crm/market-segments',
        name: '::Menu:MarketSegments',
        parentName: '::Menu:CRM',
        iconClass: 'fas fa-layer-group',
        order: 10,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Leads',
      },
      {
        path: '/crm/industry-types',
        name: '::Menu:IndustryTypes',
        parentName: '::Menu:CRM',
        iconClass: 'fas fa-industry',
        order: 11,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Leads',
      },
      {
        path: '/crm/sales-stages',
        name: '::Menu:SalesStages',
        parentName: '::Menu:CRM',
        iconClass: 'fas fa-diagram-successor',
        order: 12,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Leads',
      },
      {
        path: '/crm/appointment-settings',
        name: '::Menu:AppointmentBookingSettings',
        parentName: '::Menu:CRM',
        iconClass: 'fas fa-cog',
        order: 11,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Leads',
      },
      {
        path: '/crm/opportunity-types',
        name: '::Menu:OpportunityTypes',
        parentName: '::Menu:CRM',
        iconClass: 'fas fa-lightbulb',
        order: 12.5,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.OpportunityTypes',
      },
      {
        path: '/crm/opportunity-lost-reasons',
        name: '::Menu:OpportunityLostReasons',
        parentName: '::Menu:CRM',
        iconClass: 'fas fa-ban',
        order: 13,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.OpportunityLostReasons',
      },
      {
        path: '/crm/settings',
        name: '::Menu:CrmSettings',
        parentName: '::Menu:CRM',
        iconClass: 'fas fa-cog',
        order: 14,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.CrmSettings',
      },
      {
        path: '',
        name: '::Menu:Projects',
        iconClass: 'fas fa-project-diagram',
        order: 9,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Projects',
      },
      {
        path: '/projects',
        name: '::Menu:ProjectList',
        iconClass: 'fas fa-list',
        order: 1,
        parentName: '::Menu:Projects',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Projects',
      },
      {
        path: '/assets',
        name: '::Menu:Assets',
        iconClass: 'fas fa-building',
        order: 10,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Assets',
      },
      {
        path: '/manufacturing/work-orders',
        name: '::Menu:Manufacturing',
        iconClass: 'fas fa-industry',
        order: 11,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Manufacturing',
      },
      {
        path: '/manufacturing/dashboard',
        name: '::Menu:ManufacturingDashboard',
        iconClass: 'fas fa-gauge-high',
        order: 1,
        parentName: '::Menu:Manufacturing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Manufacturing',
      },
      {
        path: '/manufacturing/production-plans',
        name: '::Menu:ProductionPlans',
        iconClass: 'fas fa-clipboard-list',
        order: 12,
        parentName: '::Menu:Manufacturing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.ProductionPlans',
      },
      {
        path: '/manufacturing/bom',
        name: '::Menu:BillOfMaterials',
        iconClass: 'fas fa-sitemap',
        order: 13,
        parentName: '::Menu:Manufacturing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Manufacturing',
      },
      {
        path: '/manufacturing/settings',
        name: '::Menu:ManufacturingSettings',
        iconClass: 'fas fa-sliders',
        order: 14,
        parentName: '::Menu:Manufacturing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Manufacturing',
      },
      {
        path: '/sales/installation-notes',
        name: '::Menu:InstallationNotes',
        iconClass: 'fas fa-screwdriver-wrench',
        order: 15,
        parentName: '::Menu:Sales',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.DeliveryNotes',
      },
      {
        path: '/settings/authorization-rules',
        name: '::Menu:AuthorizationRules',
        iconClass: 'fas fa-shield-halved',
        order: 3,
        parentName: '::Menu:Settings',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.ApprovalWorkflows',
      },
      {
        path: '/settings/notification-logs',
        name: '::Menu:NotificationLogs',
        iconClass: 'fas fa-bell',
        order: 4,
        parentName: '::Menu:Settings',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.AutomationRules',
      },
      {
        path: '/settings/email-templates',
        name: '::Menu:EmailTemplates',
        iconClass: 'fas fa-envelope',
        order: 5,
        parentName: '::Menu:Settings',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.AutomationRules',
      },
      {
        path: '/settings/auto-repeat',
        name: '::Menu:AutoRepeat',
        iconClass: 'fas fa-repeat',
        order: 7,
        parentName: '::Menu:Settings',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.AutomationRules',
      },
      {
        path: '/automation/bulk-transactions',
        name: '::Menu:BulkTransactionLogs',
        iconClass: 'fas fa-layer-group',
        order: 7.5,
        parentName: '::Menu:Settings',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.BulkTransactionLogs',
      },
      {
        path: '/utilities/videos',
        name: '::Menu:Videos',
        iconClass: 'fas fa-video',
        order: 7.8,
        parentName: '::Menu:Settings',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Videos',
      },
      {
        path: '/settings/einvoice',
        name: '::Menu:EInvoiceSettings',
        iconClass: 'fas fa-file-shield',
        order: 6,
        parentName: '::Menu:Settings',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.EInvoice',
      },
      {
        path: '/settings/payment-terms-templates',
        name: '::Menu:PaymentTermsTemplates',
        iconClass: 'fas fa-file-contract',
        order: 7,
        parentName: '::Menu:Settings',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/settings/document-series',
        name: '::Menu:DocumentSeries',
        iconClass: 'fas fa-hashtag',
        order: 8,
        parentName: '::Menu:Settings',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Companies',
      },
      {
        path: '/settings/print-formats',
        name: 'Print Formats',
        iconClass: 'fas fa-print',
        order: 9,
        parentName: '::Menu:Settings',
        layout: eLayoutType.application,
      },
      {
        path: '/settings/letter-heads',
        name: 'Letter Heads',
        iconClass: 'fas fa-file-signature',
        order: 10,
        parentName: '::Menu:Settings',
        layout: eLayoutType.application,
      },
      {
        path: '/settings/terms-and-conditions',
        name: '::Menu:TermsAndConditions',
        iconClass: 'fas fa-file-contract',
        order: 11,
        parentName: '::Menu:Settings',
        requiredPolicy: 'MyERP.TermsAndConditions',
        layout: eLayoutType.application,
      },
      {
        path: '/settings/customer-groups',
        name: '::Menu:CustomerGroups',
        iconClass: 'fas fa-users-viewfinder',
        order: 12,
        parentName: '::Menu:Settings',
        requiredPolicy: 'MyERP.CustomerGroups',
        layout: eLayoutType.application,
      },
      {
        path: '/settings/supplier-groups',
        name: '::Menu:SupplierGroups',
        iconClass: 'fas fa-truck-ramp-box',
        order: 13,
        parentName: '::Menu:Settings',
        requiredPolicy: 'MyERP.SupplierGroups',
        layout: eLayoutType.application,
      },
      {
        path: '/settings/territories',
        name: '::Menu:Territories',
        iconClass: 'fas fa-map-location-dot',
        order: 14,
        parentName: '::Menu:Settings',
        requiredPolicy: 'MyERP.Territories',
        layout: eLayoutType.application,
      },
      {
        path: '/settings/party-types',
        name: '::Menu:PartyTypes',
        iconClass: 'fas fa-address-card',
        order: 14.5,
        parentName: '::Menu:Settings',
        requiredPolicy: 'MyERP.PartyTypes',
        layout: eLayoutType.application,
      },
      {
        path: '/sales/quotation-lost-reasons',
        name: '::Menu:QuotationLostReasons',
        iconClass: 'fas fa-flag',
        order: 15,
        parentName: '::Menu:Settings',
        requiredPolicy: 'MyERP.QuotationLostReasons',
        layout: eLayoutType.application,
      },
      {
        path: '/hr/leave-types',
        name: '::Menu:LeaveTypes',
        iconClass: 'fas fa-calendar-minus',
        order: 20,
        parentName: '::Menu:HR',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Employees',
      },
      {
        path: '/hr/salary-components',
        name: '::Menu:SalaryComponents',
        iconClass: 'fas fa-money-check',
        order: 21,
        parentName: '::Menu:HR',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Payroll',
      },
      {
        path: '/inventory/putaway-rules',
        name: '::Menu:PutawayRules',
        iconClass: 'fas fa-warehouse',
        order: 11,
        parentName: '::Menu:Inventory',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Warehouses',
      },
      {
        path: '/accounting/payment-requests',
        name: '::Menu:PaymentRequests',
        iconClass: 'fas fa-credit-card',
        order: 22,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PaymentEntries',
      },
      {
        path: '/support/issues',
        name: '::Menu:Support',
        iconClass: 'fas fa-headset',
        order: 13,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Issues',
      },
      {
        path: '/support/service-level-agreements',
        name: '::Menu:ServiceLevelAgreements',
        iconClass: 'fas fa-stopwatch',
        order: 1,
        parentName: '::Menu:Support',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.ServiceLevelAgreements',
      },
      {
        path: '/support/issue-priorities',
        name: '::Menu:IssuePriorities',
        iconClass: 'fas fa-exclamation-circle',
        order: 2,
        parentName: '::Menu:Support',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.IssuePriorities',
      },
      {
        path: '/support/issue-types',
        name: '::Menu:IssueTypes',
        iconClass: 'fas fa-tags',
        order: 3,
        parentName: '::Menu:Support',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.IssueTypes',
      },
      {
        path: '/communication/communication-media',
        name: '::Menu:CommunicationMedia',
        iconClass: 'fas fa-headset',
        order: 3.5,
        parentName: '::Menu:Support',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.CommunicationMedia',
      },
      {
        path: '/telephony/call-logs',
        name: '::Menu:CallLogs',
        iconClass: 'fas fa-phone-alt',
        order: 3.6,
        parentName: '::Menu:Support',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.CallLogs',
      },
      {
        path: '/telephony/call-types',
        name: '::Menu:TelephonyCallTypes',
        iconClass: 'fas fa-phone-square',
        order: 3.7,
        parentName: '::Menu:Support',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.TelephonyCallTypes',
      },
      {
        path: '/telephony/settings',
        name: '::Menu:IncomingCallSettings',
        iconClass: 'fas fa-phone-volume',
        order: 3.8,
        parentName: '::Menu:Support',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.IncomingCallSettings',
      },
      {
        path: '/support/settings',
        name: '::Menu:SupportSettings',
        iconClass: 'fas fa-cog',
        order: 4,
        parentName: '::Menu:Support',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SupportSettings',
      },
      {
        path: '/projects/timesheets',
        name: '::Menu:Timesheets',
        iconClass: 'fas fa-clock',
        order: 14,
        parentName: '::Menu:Projects',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Projects',
      },
      {
        path: '/projects/templates',
        name: '::Menu:ProjectTemplates',
        iconClass: 'fas fa-diagram-project',
        order: 15,
        parentName: '::Menu:Projects',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Projects',
      },
      {
        path: '/projects/activity-types',
        name: '::Menu:ActivityTypes',
        iconClass: 'fas fa-list-check',
        order: 16,
        parentName: '::Menu:Projects',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Projects',
      },
      {
        path: '/projects/activity-costs',
        name: '::Menu:ActivityCosts',
        iconClass: 'fas fa-coins',
        order: 17,
        parentName: '::Menu:Projects',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.ActivityCosts',
      },
      {
        path: '/projects/timesheet-billing',
        name: '::Menu:TimesheetBilling',
        iconClass: 'fas fa-file-invoice-dollar',
        order: 15,
        parentName: '::Menu:Projects',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Projects',
      },
      {
        path: '/projects/project-types',
        name: '::Menu:ProjectTypes',
        iconClass: 'fas fa-diagram-project',
        order: 17,
        parentName: '::Menu:Projects',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Projects',
      },
      {
        path: '/projects/task-types',
        name: '::Menu:TaskTypes',
        iconClass: 'fas fa-tasks',
        order: 18,
        parentName: '::Menu:Projects',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.TaskTypes',
      },
      {
        path: '/projects/project-updates',
        name: '::Menu:ProjectUpdates',
        iconClass: 'fas fa-bullhorn',
        order: 19,
        parentName: '::Menu:Projects',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.ProjectUpdates',
      },
      {
        path: '/hr/leave',
        name: '::Menu:Leave',
        iconClass: 'fas fa-calendar-minus',
        order: 16,
        parentName: '::Menu:HR',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Employees',
      },
      {
        path: '/hr/attendance',
        name: '::Menu:Attendance',
        iconClass: 'fas fa-calendar-day',
        order: 22,
        parentName: '::Menu:HR',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Employees',
      },
      {
        path: '/hr/shift-types',
        name: '::Menu:ShiftTypes',
        iconClass: 'fas fa-clock',
        order: 23,
        parentName: '::Menu:HR',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Employees',
      },
      {
        path: '/hr/shift-assignments',
        name: '::Menu:ShiftAssignments',
        iconClass: 'fas fa-calendar-check',
        order: 24,
        parentName: '::Menu:HR',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Employees',
      },
      {
        path: '/hr/holiday-lists',
        name: '::Menu:HolidayLists',
        iconClass: 'fas fa-calendar-days',
        order: 16,
        parentName: '::Menu:HR',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Employees',
      },
      {
        path: '/hr/salary-structures',
        name: '::Menu:SalaryStructures',
        iconClass: 'fas fa-money-bill-wave',
        order: 17,
        parentName: '::Menu:HR',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Payroll',
      },
      {
        path: '/hr/leave-allocations',
        name: '::Menu:LeaveAllocations',
        iconClass: 'fas fa-calendar-check',
        order: 18,
        parentName: '::Menu:HR',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Employees.Edit',
      },
      {
        path: '/hr/loans',
        name: '::Menu:Loans',
        iconClass: 'fas fa-hand-holding-dollar',
        order: 19,
        parentName: '::Menu:HR',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Employees',
      },
      {
        path: '/manufacturing/workstation-capacity',
        name: '::Menu:WorkstationCapacity',
        iconClass: 'fas fa-chart-simple',
        order: 4,
        parentName: '::Menu:Manufacturing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Manufacturing',
      },
      {
        path: '/manufacturing/workstations',
        name: '::Menu:Workstations',
        iconClass: 'fas fa-gear',
        order: 5,
        parentName: '::Menu:Manufacturing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Manufacturing',
      },
      {
        path: '/manufacturing/operations',
        name: '::Menu:Operations',
        iconClass: 'fas fa-wrench',
        order: 11,
        parentName: '::Menu:Manufacturing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Manufacturing',
      },
      {
        path: '/manufacturing/routings',
        name: '::Menu:Routings',
        iconClass: 'fas fa-route',
        order: 12,
        parentName: '::Menu:Manufacturing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Manufacturing',
      },
      {
        path: '/manufacturing/workstation-types',
        name: '::Menu:WorkstationTypes',
        iconClass: 'fas fa-industry',
        order: 13,
        parentName: '::Menu:Manufacturing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Manufacturing',
      },
      {
        path: '/manufacturing/job-cards',
        name: '::Menu:JobCards',
        iconClass: 'fas fa-id-card',
        order: 6,
        parentName: '::Menu:Manufacturing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Manufacturing',
      },
      {
        path: '/manufacturing/downtime-entries',
        name: '::Menu:DowntimeEntries',
        iconClass: 'fas fa-hourglass-half',
        order: 7,
        parentName: '::Menu:Manufacturing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Manufacturing',
      },
      {
        path: '/manufacturing/bom-creators',
        name: '::Menu:BomCreators',
        iconClass: 'fas fa-diagram-project',
        order: 8,
        parentName: '::Menu:Manufacturing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Manufacturing',
      },
      {
        path: '/manufacturing/master-production-schedules',
        name: '::Menu:MasterProductionSchedules',
        iconClass: 'fas fa-calendar-days',
        order: 9,
        parentName: '::Menu:Manufacturing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.MasterProductionSchedules',
      },
      {
        path: '/manufacturing/sales-forecasts',
        name: '::Menu:SalesForecasts',
        iconClass: 'fas fa-chart-line',
        order: 10,
        parentName: '::Menu:Manufacturing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesForecasts',
      },
      {
        path: '/manufacturing/reports/analytics',
        name: '::Menu:ProductionAnalytics',
        iconClass: 'fas fa-chart-bar',
        order: 15,
        parentName: '::Menu:Manufacturing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Manufacturing',
      },
      {
        path: '/manufacturing/reports/bom-stock-analysis',
        name: '::Menu:BomStockAnalysis',
        iconClass: 'fas fa-sitemap',
        order: 16,
        parentName: '::Menu:Manufacturing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Manufacturing',
      },
      {
        path: '/manufacturing/plant-floors',
        name: '::Menu:PlantFloors',
        iconClass: 'fas fa-building',
        order: 17,
        parentName: '::Menu:Manufacturing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PlantFloors',
      },
      {
        path: '/sales/proforma-invoices',
        name: '::Menu:ProformaInvoices',
        iconClass: 'fas fa-file-invoice',
        order: 14,
        parentName: '::Menu:Sales',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesInvoices',
      },
      {
        path: '/sales/blanket-orders',
        name: '::Menu:BlanketOrders',
        iconClass: 'fas fa-file-contract',
        order: 15,
        parentName: '::Menu:Sales',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesOrders',
      },
      {
        path: '/sales/subscriptions',
        name: '::Menu:Subscriptions',
        iconClass: 'fas fa-rotate',
        order: 16,
        parentName: '::Menu:Sales',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesInvoices',
      },
      {
        path: '/sales/pricing-rules',
        name: '::Menu:PricingRules',
        iconClass: 'fas fa-tags',
        order: 17,
        parentName: '::Menu:Sales',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesInvoices',
      },
      {
        path: '/sales/promotional-schemes',
        name: '::Menu:PromotionalSchemes',
        iconClass: 'fas fa-gifts',
        order: 18,
        parentName: '::Menu:Sales',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PromotionalSchemes',
      },
      {
        path: '/purchasing/supplier-quotations',
        name: '::Menu:SupplierQuotations',
        iconClass: 'fas fa-file-invoice-dollar',
        order: 8,
        parentName: '::Menu:Purchasing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PurchaseOrders',
      },
      {
        path: '/purchasing/subcontracting',
        name: '::Menu:Subcontracting',
        iconClass: 'fas fa-handshake',
        order: 9,
        parentName: '::Menu:Purchasing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PurchaseOrders',
      },
      {
        path: '/purchasing/subcontracting-inward',
        name: '::Menu:SubcontractingInwardOrders',
        iconClass: 'fas fa-truck-ramp-box',
        order: 11,
        parentName: '::Menu:Purchasing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PurchaseOrders',
      },
      {
        path: '/purchasing/rfq',
        name: '::Menu:RequestForQuotations',
        iconClass: 'fas fa-file-circle-question',
        order: 12,
        parentName: '::Menu:Purchasing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PurchaseOrders',
      },
      {
        path: '/purchasing/scorecards',
        name: '::Menu:SupplierScorecards',
        iconClass: 'fas fa-star-half-stroke',
        order: 11,
        parentName: '::Menu:Purchasing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SupplierScorecards',
      },
      {
        path: '/hr/expense-claims',
        name: '::Menu:ExpenseClaims',
        iconClass: 'fas fa-receipt',
        order: 18,
        parentName: '::Menu:HR',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Employees',
      },
      {
        path: '/hr/salary-slips',
        name: '::Menu:SalarySlips',
        iconClass: 'fas fa-file-invoice-dollar',
        order: 19,
        parentName: '::Menu:HR',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Payroll',
      },
      {
        path: '/sales/dunnings',
        name: '::Menu:Dunnings',
        iconClass: 'fas fa-bell',
        order: 18,
        parentName: '::Menu:Sales',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesInvoices',
      },
      {
        path: '/sales/dunning-types',
        name: '::Menu:DunningTypes',
        iconClass: 'fas fa-file-invoice-dollar',
        order: 19,
        parentName: '::Menu:Sales',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesInvoices',
      },
      {
        path: '/inventory/serial-numbers',
        name: '::Menu:SerialNumbers',
        iconClass: 'fas fa-barcode',
        order: 7,
        parentName: '::Menu:Inventory',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Items',
      },
      {
        path: '/inventory/batches',
        name: '::Menu:Batches',
        iconClass: 'fas fa-boxes-stacked',
        order: 8,
        parentName: '::Menu:Inventory',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Items',
      },
      {
        path: '/accounting/reports/aging',
        name: '::Menu:AgingReport',
        iconClass: 'fas fa-clock',
        order: 14,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesInvoices',
      },
      {
        path: '/accounting/reports/general-ledger',
        name: '::Menu:GeneralLedger',
        iconClass: 'fas fa-book-open',
        order: 15,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/reports/outstanding',
        name: '::Menu:OutstandingInvoices',
        iconClass: 'fas fa-file-circle-exclamation',
        order: 16,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesInvoices',
      },
      {
        path: '/accounting/reports/party-ledger',
        name: '::Menu:PartyLedger',
        iconClass: 'fas fa-address-book',
        order: 17,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/reports/statement-of-accounts',
        name: '::Menu:StatementOfAccounts',
        iconClass: 'fas fa-file-contract',
        order: 18,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/reports/cash-flow-forecast',
        name: '::Menu:CashFlowForecast',
        iconClass: 'fas fa-chart-area',
        order: 19,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/currency-exchange',
        name: '::Menu:CurrencyExchange',
        iconClass: 'fas fa-exchange-alt',
        order: 20,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/finance-books',
        name: '::Menu:FinanceBooks',
        iconClass: 'fas fa-book-bookmark',
        order: 20,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/closing-balances',
        name: '::Menu:ClosingBalances',
        iconClass: 'fas fa-scale-balanced',
        order: 23,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/gl-repost',
        name: '::Menu:GLRepost',
        iconClass: 'fas fa-rotate',
        order: 24,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/payment-ledger-repost',
        name: '::Menu:PaymentLedgerRepost',
        iconClass: 'fas fa-rotate',
        order: 25,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/ledger-health-monitor',
        name: '::Menu:LedgerHealthMonitor',
        iconClass: 'fas fa-heart-pulse',
        order: 26,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/journal-entry-templates',
        name: '::Menu:JournalEntryTemplates',
        iconClass: 'fas fa-file-invoice',
        order: 27,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/bank-transaction-rules',
        name: '::Menu:BankTransactionRules',
        iconClass: 'fas fa-robot',
        order: 21,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PaymentEntries',
      },
      {
        path: '/accounting/banks',
        name: '::Menu:Banks',
        iconClass: 'fas fa-university',
        order: 21.5,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Banks',
      },
      {
        path: '/accounting/cheque-print-templates',
        name: '::Menu:ChequePrintTemplates',
        iconClass: 'fas fa-money-check',
        order: 21.6,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.ChequePrintTemplates',
      },
      {
        path: '/accounting/bank-account-types',
        name: '::Menu:BankAccountTypes',
        iconClass: 'fas fa-tags',
        order: 22,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.BankAccountTypes',
      },
      {
        path: '/accounting/bank-account-subtypes',
        name: '::Menu:BankAccountSubtypes',
        iconClass: 'fas fa-tag',
        order: 23,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.BankAccountSubtypes',
      },
      {
        path: '/accounting/bank-account-balances',
        name: '::Menu:BankAccountBalances',
        iconClass: 'fas fa-chart-line',
        order: 23.5,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.BankAccountBalances',
      },
      {
        path: '/accounting/exchange-rate-revaluation',
        name: '::Menu:ExchangeRateRevaluation',
        iconClass: 'fas fa-scale-unbalanced',
        order: 21,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/payment-orders',
        name: '::Menu:PaymentOrders',
        iconClass: 'fas fa-money-check-alt',
        order: 22,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PaymentOrders',
      },
      {
        path: '/accounting/unreconcile-payments',
        name: '::Menu:UnreconcilePayments',
        iconClass: 'fas fa-unlink',
        order: 23,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.UnreconcilePayments',
      },
      {
        path: '/accounting/party-links',
        name: '::Menu:PartyLinks',
        iconClass: 'fas fa-people-arrows',
        order: 24,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/invoice-discounting',
        name: '::Menu:InvoiceDiscounting',
        iconClass: 'fas fa-percent',
        order: 22,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesInvoices',
      },
      {
        path: '/accounting/repost-accounting-ledger',
        name: '::Menu:RepostAccountingLedger',
        iconClass: 'fas fa-sync',
        order: 28,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.RepostAccountingLedger',
      },
      {
        path: '/accounting/process-payment-reconciliation',
        name: '::Menu:ProcessPaymentReconciliation',
        iconClass: 'fas fa-robot',
        order: 29,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.ProcessPaymentReconciliation',
      },
      {
        path: '/accounting/fiscal-years',
        name: '::Menu:FiscalYears',
        iconClass: 'fas fa-calendar-days',
        order: 17,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/period-closing',
        name: '::Menu:PeriodClosing',
        iconClass: 'fas fa-lock',
        order: 18,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/month-end-close',
        name: '::Menu:MonthEndClose',
        iconClass: 'fas fa-calendar-check',
        order: 19,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/cost-center-allocations',
        name: '::Menu:CostCenterAllocations',
        iconClass: 'fas fa-diagram-project',
        order: 21,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/report-templates',
        name: '::Menu:ReportTemplates',
        iconClass: 'fas fa-file-lines',
        order: 22,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/report-templates/execute',
        name: '::Menu:RunFinancialReport',
        iconClass: 'fas fa-chart-bar',
        order: 23,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/account-categories',
        name: '::Menu:AccountCategories',
        iconClass: 'fas fa-layer-group',
        order: 24,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/opening-balance',
        name: '::Menu:OpeningBalance',
        iconClass: 'fas fa-scale-balanced',
        order: 19,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.JournalEntries.Create',
      },
      {
        path: '/accounting/dimensions',
        name: '::Menu:AccountingDimensions',
        iconClass: 'fas fa-cubes',
        order: 20,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/settings',
        name: '::Menu:AccountsSettings',
        iconClass: 'fas fa-sliders',
        order: 25,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.AccountsSettings',
      },
      {
        path: '/accounting/currency-exchange-settings',
        name: '::Menu:CurrencyExchangeSettings',
        iconClass: 'fas fa-money-bill-transfer',
        order: 26,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.CurrencyExchangeSettings',
      },
      {
        path: '/accounting/cashier-closings',
        name: '::Menu:CashierClosing',
        iconClass: 'fas fa-cash-register',
        order: 27,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.CashierClosings',
      },
      {
        path: '/accounting/batch-payment',
        name: '::Menu:BatchPayment',
        iconClass: 'fas fa-money-check-dollar',
        order: 20,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PaymentEntries.Create',
      },
      {
        path: '/accounting/reports/cash-flow',
        name: '::Menu:CashFlowStatement',
        iconClass: 'fas fa-water',
        order: 21,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/accounting/reports/budget-variance',
        name: '::Menu:BudgetVarianceReport',
        iconClass: 'fas fa-chart-column',
        order: 22,
        parentName: '::Menu:Accounting',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Accounts',
      },
      {
        path: '/sales/reports/gross-profit',
        name: '::Menu:GrossProfitReport',
        iconClass: 'fas fa-chart-line',
        order: 19,
        parentName: '::Menu:Sales',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesInvoices',
      },
      {
        path: '/sales/reports/register',
        name: '::Menu:SalesRegister',
        iconClass: 'fas fa-table-list',
        order: 20,
        parentName: '::Menu:Sales',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesInvoices',
      },
      {
        path: '/sales/reports/item-sales',
        name: '::Menu:ItemSalesSummary',
        iconClass: 'fas fa-chart-column',
        order: 21,
        parentName: '::Menu:Sales',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesInvoices',
      },
      {
        path: '/sales/reports/analytics',
        name: '::Menu:SalesAnalytics',
        iconClass: 'fas fa-chart-bar',
        order: 23,
        parentName: '::Menu:Sales',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesInvoices',
      },
      {
        path: '/sales/reports/customer-revenue',
        name: '::Menu:CustomerRevenue',
        iconClass: 'fas fa-users-line',
        order: 22,
        parentName: '::Menu:Sales',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesInvoices',
      },
      {
        path: '/sales/reports/commission',
        name: '::Menu:SalesCommission',
        iconClass: 'fas fa-coins',
        order: 23,
        parentName: '::Menu:Sales',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesOrders',
      },
      {
        path: '/sales/reports/pending-delivery',
        name: '::Menu:PendingDelivery',
        iconClass: 'fas fa-truck-clock',
        order: 23,
        parentName: '::Menu:Sales',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.SalesOrders',
      },
      {
        path: '/purchasing/reports/register',
        name: '::Menu:PurchaseRegister',
        iconClass: 'fas fa-table-list',
        order: 10,
        parentName: '::Menu:Purchasing',
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.PurchaseInvoices',
      },
      {
        path: '/settings/company',
        name: '::Menu:CompanySettings',
        iconClass: 'fas fa-gear',
        order: 100,
        layout: eLayoutType.application,
      },
      // Assets Module
      {
        path: '/assets',
        name: '::Menu:Assets',
        iconClass: 'fas fa-boxes-stacked',
        order: 65,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Assets',
      },
      {
        path: '/assets',
        name: '::Menu:AssetList',
        parentName: '::Menu:Assets',
        iconClass: 'fas fa-list',
        order: 1,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Assets',
      },
      {
        path: '/assets/maintenances',
        name: '::Menu:AssetMaintenances',
        parentName: '::Menu:Assets',
        iconClass: 'fas fa-cogs',
        order: 2,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.AssetMaintenances',
      },
      {
        path: '/assets/maintenance-teams',
        name: '::Menu:AssetMaintenanceTeams',
        parentName: '::Menu:Assets',
        iconClass: 'fas fa-people-group',
        order: 3,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.AssetMaintenances',
      },
      {
        path: '/assets/maintenance-logs',
        name: '::Menu:AssetMaintenanceLogs',
        parentName: '::Menu:Assets',
        iconClass: 'fas fa-calendar-check',
        order: 4,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.AssetMaintenanceLogs',
      },
      {
        path: '/assets/repairs',
        name: '::Menu:AssetRepairs',
        parentName: '::Menu:Assets',
        iconClass: 'fas fa-wrench',
        order: 4,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.AssetRepairs',
      },
      {
        path: '/assets/capitalizations',
        name: '::Menu:AssetCapitalizations',
        parentName: '::Menu:Assets',
        iconClass: 'fas fa-coins',
        order: 5,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.AssetCapitalizations',
      },
      {
        path: '/assets/categories',
        name: '::Menu:AssetCategories',
        parentName: '::Menu:Assets',
        iconClass: 'fas fa-tags',
        order: 6,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Assets',
      },
      {
        path: '/assets/locations',
        name: '::Menu:Locations',
        parentName: '::Menu:Assets',
        iconClass: 'fas fa-map-marker-alt',
        order: 7,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Locations',
      },
      {
        path: '/assets/vehicles',
        name: '::Menu:Vehicles',
        parentName: '::Menu:Assets',
        iconClass: 'fas fa-truck',
        order: 8,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Vehicles',
      },
      {
        path: '/assets/drivers',
        name: '::Menu:Drivers',
        parentName: '::Menu:Assets',
        iconClass: 'fas fa-id-badge',
        order: 9,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Drivers',
      },
      {
        path: '/assets/driving-license-categories',
        name: '::Menu:DrivingLicenseCategories',
        parentName: '::Menu:Assets',
        iconClass: 'fas fa-id-card',
        order: 10,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.DrivingLicenseCategories',
      },
      {
        path: '/assets/shift-factors',
        name: '::Menu:AssetShiftFactors',
        parentName: '::Menu:Assets',
        iconClass: 'fas fa-clock',
        order: 10,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.AssetCategories',
      },
      {
        path: '/assets/shift-allocations',
        name: '::Menu:AssetShiftAllocations',
        parentName: '::Menu:Assets',
        iconClass: 'fas fa-arrows-spin',
        order: 11,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.Assets',
      },
      {
        path: '/assets/movements',
        name: '::Menu:AssetMovements',
        parentName: '::Menu:Assets',
        iconClass: 'fas fa-truck-ramp-box',
        order: 8,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.AssetMovements',
      },
      {
        path: '/assets/value-adjustments',
        name: '::Menu:AssetValueAdjustments',
        parentName: '::Menu:Assets',
        iconClass: 'fas fa-sliders',
        order: 9,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.AssetValueAdjustments',
      },
      // Maintenance & Support Module
      {
        path: '/maintenance',
        name: '::Menu:Maintenance',
        iconClass: 'fas fa-screwdriver-wrench',
        order: 66,
        layout: eLayoutType.application,
      },
      {
        path: '/maintenance/warranty-claims',
        name: '::Menu:WarrantyClaims',
        parentName: '::Menu:Maintenance',
        iconClass: 'fas fa-shield-halved',
        order: 1,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.WarrantyClaims',
      },
      {
        path: '/maintenance/schedules',
        name: '::Menu:MaintenanceSchedules',
        parentName: '::Menu:Maintenance',
        iconClass: 'fas fa-calendar-days',
        order: 2,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.MaintenanceSchedules',
      },
      {
        path: '/maintenance/visits',
        name: '::Menu:MaintenanceVisits',
        parentName: '::Menu:Maintenance',
        iconClass: 'fas fa-user-gear',
        order: 3,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.MaintenanceVisits',
      },
      // LHDN e-Invoice Module
      {
        path: '/e-invoice/dashboard',
        name: '::Menu:EInvoice',
        iconClass: 'fas fa-file-invoice',
        order: 90,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.EInvoice',
      },
      {
        path: '/e-invoice/dashboard',
        name: '::Menu:EInvoiceDashboard',
        parentName: '::Menu:EInvoice',
        iconClass: 'fas fa-chart-line',
        order: 1,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.EInvoice',
      },
      {
        path: '/einvoice/batch-submit',
        name: '::Menu:EInvoiceBatchSubmit',
        parentName: '::Menu:EInvoice',
        iconClass: 'fas fa-paper-plane',
        order: 2,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.EInvoice',
      },
      {
        path: '/e-invoice/consolidation',
        name: '::Menu:EInvoiceConsolidation',
        parentName: '::Menu:EInvoice',
        iconClass: 'fas fa-layer-group',
        order: 3,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.EInvoice',
      },
      {
        path: '/e-invoice/reports',
        name: '::Menu:EInvoiceReports',
        parentName: '::Menu:EInvoice',
        iconClass: 'fas fa-chart-pie',
        order: 4,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.EInvoice',
      },
      {
        path: '/e-invoice/logs',
        name: '::Menu:EInvoiceLogs',
        parentName: '::Menu:EInvoice',
        iconClass: 'fas fa-clipboard-list',
        order: 5,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.EInvoice',
      },
      {
        path: '/einvoice/settings',
        name: '::Menu:EInvoiceSettings',
        parentName: '::Menu:EInvoice',
        iconClass: 'fas fa-cog',
        order: 6,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.EInvoice',
      },
      {
        path: '/edi/code-lists',
        name: '::Menu:CodeLists',
        parentName: '::Menu:EInvoice',
        iconClass: 'fas fa-list-alt',
        order: 7,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.CodeLists',
      },
      {
        path: '/edi/common-codes',
        name: '::Menu:CommonCodes',
        parentName: '::Menu:EInvoice',
        iconClass: 'fas fa-barcode',
        order: 8,
        layout: eLayoutType.application,
        requiredPolicy: 'MyERP.CommonCodes',
      },
    ]);
}
