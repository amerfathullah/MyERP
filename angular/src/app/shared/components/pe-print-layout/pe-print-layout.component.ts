      ...
      <div class="pe-print-layout">
        ...
        <div class="print-header d-flex justify-content-between align-items-start mb-4">
          ...
          <div class="text-end">
            ...
            @if (payment.referenceNumber) {
              <p class="mb-0"><strong>Reference:</strong> {{ payment.referenceNumber }}</p>
            }
          </div>
        </div>
        <hr class="mb-4">
        ...
        <div class="row mb-4">
          <div class="col-6">
            ...
            @if (partyAddress) {
              <p class="text-muted mb-0">{{ partyAddress }}</p>
            }
          </div>
          <div class="col-6">
            ...
            @if (payment.referenceNumber) {
              <p class="text-muted mb-0">Ref: {{ payment.referenceNumber }}</p>
            }
            @if (payment.referenceDate) {
              <p class="text-muted mb-0">Cheque Date: {{ payment.referenceDate | date:'dd/MM/yyyy' }}</p>
            }
          </div>
        </div>
        <!-- Amount Hero -->
        <div class="text-center border rounded p-3 mb-4 bg-light">
          ...
          <h2 class="fw-bold mb-1">{{ payment.currency || 'IQD' }} {{ payment.paidAmount | number:'1.2-2' }}</h2>
          @if (amountInWords) {
            <p class="text-muted mb-0 fst-italic">{{ amountInWords }}</p>
          }
        </div>
        ...
        @if (payment.references && payment.references.length > 0) {
          <div class="mb-4">
            ...
            <table class="table table-bordered table-sm">
              ...
              <tbody>
                @for (ref of payment.references; track ref; let i = $index) {
                  <tr>
                    <td>{{ i + 1 }}</td>
        ...
        @if (!payment.references?.length && payment.againstInvoiceId) {
          <div class="mb-4">
        ...
        @if (payment.exchangeRate && payment.exchangeRate !== 1) {
          <div class="mb-4">
            ...
            <p class="mb-0">1 {{ payment.currency }} = {{ payment.exchangeRate | number:'1.4-4' }} IQD</p>
            <p class="mb-0">Base Amount: IQD {{ baseAmount | number:'1.2-2' }}</p>
          </div>
        }
        <!-- Remarks -->
        @if (payment.notes) {
