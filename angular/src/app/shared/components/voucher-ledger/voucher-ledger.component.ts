...
@Component({
  ...
  template: `
    <div class="card mt-3" *ngIf="isVisible()">
...
export class VoucherLedgerComponent implements OnChanges {
  ...
  private loadGlEntries(): void {
    ...
    this.http.get<any>(`/api/app/general-ledger/for-voucher`, {
      params: { voucherType: this.voucherType, voucherId: this.voucherId }
    }).subscribe({
      next: (data) => {
  ...
  private loadSleEntries(): void {
    this.loading.set(true);
    this.http.get<any>(`/api/app/stock-ledger/for-voucher`, {
      params: { voucherType: this.voucherType, voucherId: this.voucherId }
    }).subscribe({
      next: (data) => {
